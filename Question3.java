...
// Question 3 - extends Question 2 by adding grades
// Subjects: 1 = Maths, 2 = Chemistry, 3 = Physics
// Grading: 90+ = A, 80-89 = B, 70-79 = C, 60-69 = D, below 60 = Fail
public class Question3 {
    ...
    public static void main(String[] args) {
        ...
        while (running) {
            ...
            System.out.println("6. Show grades table");
            ...
            switch (choice) {
                ...
                case "6": // grades table - this is the Question 3 part
                    boolean any = false;
                    for (int id = 1; id <= n; id++) if (added[id]) any = true;
                    if (!any) {
                        System.out.println("No marks entered yet.");
                        break;
                    }
                    System.out.println("\nID\tMaths\tGrade\tChem\tGrade\tPhysics\tGrade");
                    for (int id = 1; id <= n; id++) {
                        if (!added[id]) continue;
                        System.out.println(id + "\t" + marks[id][1] + "\t" + grade(marks[id][1])
                                + "\t" + marks[id][2] + "\t" + grade(marks[id][2])
                                + "\t" + marks[id][3] + "\t" + grade(marks[id][3]));
                    }
                    break;
    ...
    // Question 3 - works out the letter grade for one mark
